...
_deny = msg {
 ...
 msg := sprintf("%s/%s: API apiextensions.k8s.io/v1beta1 for CustomResourceDefinition is deprecated in 1.19, use apiextensions.k8s.io/v1 instead, available since v1.16", [input.kind, input.metadata.name])
}
...
_deny = msg {
  ...
  msg := sprintf("%s/%s: API apiregistration.k8s.io/v1beta1 is deprecated in Kubernetes 1.19, use apiregistration.k8s.io/v1 instead, available since v1.10", [input.kind, input.metadata.name])
}
...
_deny = msg {
  ...
  msg := sprintf("%s/%s: API authentication.k8s.io/v1beta1 is deprecated in Kubernetes 1.19, use authentication.k8s.io/v1 instead, available since v1.16", [input.kind, input.metadata.name])
}
...
_warn = msg {
  ...
  msg := sprintf("%s/%s: API authorization.k8s.io/v1beta1 is deprecated in Kubernetes 1.19, use authorization.k8s.io/v1 instead, available since v1.6", [input.kind, input.metadata.name])
}
...
_deny = msg {
  ...
  msg := sprintf("%s/%s: API coordination.k8s.io/v1beta1 is deprecated in Kubernetes 1.19, use coordination.k8s.io/v1 instead, available since v1.14", [input.kind, input.metadata.name])
}
...
#  input.kind == "Ingress"
#  msg := sprintf("%s/%s: API extensions/v1beta1 for Ingress is deprecated from Kubernetes 1.19, use networking.k8s.io/v1 instead, available since v1.19.", [input.kind, input.metadata.name])
#}
...
_deny = msg {
  apis := ["networking.k8s.io/v1beta1","extensions/v1beta1"]
  input.kind == apis[_]
  msg := sprintf("%s/%s: API networking.k8s.io/v1beta1 and storage.k8s.io/v1beta1 for Ingress is deprecated from Kubernetes 1.19, use networking.k8s.io/v1 instead, available since v1.19.", [input.kind, input.metadata.name])
}
